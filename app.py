...
# === Like API ===
async def send_like(uid: str, target_uid: str, region: str):
    try:
        payload_dict = {"uid": uid, "target_uid": target_uid, "type": 1}
        payload_bytes = await json_to_proto(json.dumps(payload_dict), main_pb2.SendLikeReq())
        data_enc = aes_cbc_encrypt(MAIN_KEY, MAIN_IV, payload_bytes)
        token, lock, server = await get_token_info(region)
        headers = {
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(server + "/SendLike", data=data_enc, headers=headers)
            decoded = decode_protobuf(resp.content, main_pb2.SendLikeRes())
            result = json.loads(json_format.MessageToJson(decoded))
            return {"success": True, "data": result}
    except Exception as e:
        return {"success": False, "error": str(e)}

@app.route('/send-like', methods=['GET'])
@cached_endpoint()
def like_endpoint():
    uid = request.args.get('uid')
    ...
    if not uid or not target_uid:
        return jsonify({"error": "Please provide uid and target_uid"}), 400
    try:
        result = asyncio.run(send_like(uid, target_uid, region))
